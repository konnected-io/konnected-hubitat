 ...
 *  Konnected Pro
 *
 *  Copyright 2020 Konnected Inc; https://konnected.io
 *
...
preferences {
  page(name: "mainPage", title: "Konnected Devices", install: true, uninstall: true) {
    section {
      app(name: "childApps", appName: "Konnected Pro Device", namespace: "konnected-io", title: "Add a Konnected Pro device", multiple: true)
      paragraph "Konnected Pro v${version()}"
    }
  }
}

def installed() {
  log.info "installed(): Installing Konnected Pro Parent App"
  initialize()
}

def updated() {
  log.info "updated(): Updating Konnected App"
  unschedule()
  initialize()
}

def uninstalled() {
  log.info "uninstall(): Uninstalling Konnected App"
}
...
def discoverySearch() {
  log.debug "Discovering Konnected Pro devices on the network via SSDP"
...
def discoveryDeviceType() {
  return "urn:schemas-konnected-io:device:Security:2"
}
